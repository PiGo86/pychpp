from typing import List, Tuple, Union, Iterable, TYPE_CHECKING
...
from pychpp.models.xml.challenges import ChallengeItem, ChallengesView, CommonRequestChallenges, ChallengesChallengeable
...
class HTChallengeManager:
    ...
    def __init__(self, chpp: 'CHPP', team_id: int = None, is_weekend_friendly: bool = None, **kwargs):
        self._chpp = chpp
    ...
    def pending_challenges(self, author: str = 'both') -> Union[List[ChallengeItem],
                                                                Tuple[List[ChallengeItem], List[ChallengeItem]]]:
        ...
        if author == 'own_team':
            ...
            raise ValueError(f"if set, author argument must be equal to 'own_team', 'other_teams', 'both'")
    ...
    def are_challengeable(
        ...
        if teams is not None:
            result = ChallengesChallengeable(
                chpp=self._chpp, action_type='challengeable', suggested_team_ids=','.join(str(t.id) for t in teams)
            ).challengeable_result

        elif team_ids is not None:
            result = ChallengesChallengeable(
                chpp=self._chpp, action_type='challengeable', suggested_team_ids=','.join(str(id_) for id_ in team_ids)
            ).challengeable_result
    ...
    def accept(self, training_match_ht_id) -> None:
        self._chpp.xml_challenges(action_type='accept', training_match_id=training_match_ht_id, **self.kwargs)

    def decline(self, training_match_ht_id) -> None:
        self._chpp.xml_challenges(action_type='decline', training_match_id=training_match_ht_id, **self.kwargs)

    def withdraw(self, training_match_ht_id) -> None:
        self._chpp.xml_challenges(action_type='withdraw', training_match_id=training_match_ht_id, **self.kwargs)
